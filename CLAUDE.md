...
- `app.js` — UI logic, list rendering, plan/log/edit/delete forms, tab switching
...
- `manifest.json` / `sw.js` / `icons/` — PWA support (added 2026-07-30). Service worker is same-origin cache-first for the app shell only; map tiles/geolocation/Nominatim calls always pass through to network. Registration is guarded to only attempt on `https:` or `localhost`.
...
- **Calendar/grid view** — Upcoming/History are chronological lists today; no true calendar widget yet.
